...
public:
    ...
    public:
        ...
        static const size_t SAMPLE_SIZE = 20;    // 采样数目（最大）。实际采样数目是2倍SAMPLE_SIZE
        ...
        ColumnSample(const Column& column, size_t row_num) : type_(column.type), page_num_(column.pages.size()){
            ...
            if(page_num_ >= SAMPLE_SIZE){
                ...
                std::vector<uint32_t> sample_row_idx(2*sample_size,0);
                for(size_t i=0; i<sample_size; i++){
    ...
    // DONT_CACHE: 不要缓存。可能该节点是探测测，或者该节点是构建侧但是输出包含VARCAHR，或者深度过深，或者被释放掉了
    // NEED_CACHE: 需要缓存。表明该节点希望得到缓存。经过查找缓存和实际执行后，变为HAS_CACHE或者OWN_CACHE
    // HAS_CACHE: 找到了缓存。此时对应的hashmaps_中存储的是被缓存的哈希表
    // OWN_CACHE: 拥有缓存。此时对应的hashmaps_中存储的是该语句执行中生成的哈希表
    enum CacheType{DONT_CACHE, NEED_CACHE, HAS_CACHE, OWN_CACHE};
    ...
    inline void setCache(size_t node_id, Hashmap* hashmap){
        cache_types_[node_id] = HAS_CACHE;
...
private:
    ...
    static const uint64_t INVALID_HASH=1;   // 无效的哈希值
    std::vector<uint64_t> hashes_;          // 每个节点的哈希值

    std::vector<CacheType> cache_types_;    // 每个节点是否需要缓存哈希表
    std::vector<Hashmap*> hashmaps_;        // 每个节点（输出）的哈希表（可能是空的）

    static const size_t MAX_NODE_DEPTH=2;   // 深度超过MAX_NODE_DEPTH的节点不缓存
    ...
    uint64_t calculateNodeHash(const Plan& plan, size_t node_id){
        ...
        return std::visit([&](auto&& node_data) -> uint64_t {
            ...
            if constexpr (std::is_same_v<T, ScanNode>) {
                ...
                // 分获取左右算子的哈希值，如果任意一侧无效，则设置该节点为无效
                uint64_t left_hash = calculateNodeHash(plan,node_data.left);
                uint64_t right_hash = calculateNodeHash(plan,node_data.right);
                // 判断构建侧算子是否需要被被缓存
                if(node_data.build_left && left_hash!=INVALID_HASH && node_depth_[node_data.left]<=MAX_NODE_DEPTH){
                    cache_types_[node_data.left]=NEED_CACHE;
                } else if(!node_data.build_left && right_hash!=INVALID_HASH && node_depth_[node_data.right]<=MAX_NODE_DEPTH){
                    cache_types_[node_data.right]=NEED_CACHE;
                }
...
class CacheManager{
    ...
    static const size_t MAX_CACHE_SIZE=1024*1024*20;    // 最多20G
...
public:
    ...
    QueryCache* getQuery(const Plan& plan){
        ...
        for(size_t i=0; i<plan.nodes.size(); i++){
            if(query->getCacheType(i)==QueryCache::NEED_CACHE){
                uint64_t node_hash = query->getNodeHash(i);
                if(caches_map_.find(node_hash)!=caches_map_.end()){
                    ...
                    if(query->isSame(i,cached_query,node_id)){
    ...
    void cacheQuery(QueryCache* query){
        ...
        while(cache_size_ + query_size > MAX_CACHE_SIZE){
            ...
            caches_map_.erase(queries_[query_id]->getNodeHash(node_id));
        ...
        // 将本查询中的OWN_CACHE的节点加入到缓存中
        for(size_t i=0; i<query->getNodeNum(); i++){
            if(query->getCacheType(i)==QueryCache::OWN_CACHE && query->getHashmap(i)!=nullptr){
                hashmap_caches_.emplace_back(queries_.size()-1,i);
                caches_map_.emplace(query->getNodeHash(i),--hashmap_caches_.end());
            }
        }
    }
